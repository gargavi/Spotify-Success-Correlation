...
axes[0].title.set_text("Lyrics")
axes[1].title.set_text("Tempo")
...
for genre, df_list in pandas_dataframes.items():
    ...
    lyr_axes[x_coor][y_coor].scatter(df_list[0]["success"], df_list[0]["lyrics"])
    lyr_axes[x_coor][y_coor].title.set_text("Total Genre")
    suc_axes[x_coor][y_coor].scatter(df_list[0]["success"], df_list[0]["tempo"])
    ...
    for index, row in artist_database.iterrows():
        ...
        try:
            ...
            lyric_axes.scatter(complete_artist["success"], complete_artist["lyrics"])
            success_axes.scatter(complete_artist["success"], complete_artist["tempo"])
